...
describe('ManageFeeds tests.', () => {
    ...
    test('Correct number of feeds listed under Remove Feeds', () => {
        ...
        const subscribedFeedsCount = Object.keys(UserFeedService.subscribedFeeds).length;
        const removeFeedsItemCount = manageFeeds.find('.subscribedFeedItem').hostNodes().length;
    ...
    test('Correct number of feeds listed under Suggested Feeds', () => {
        const manageFeeds = mount(<ManageFeeds />);
        const suggestionsDisplayedCount = manageFeeds.find('.suggestedFeedItem').hostNodes().length;
    ...
    test('Add custom feed form functioning properly', () => {
        ...
        manageFeeds.find('.feedNameInput').hostNodes().simulate('change', { target: {value: rssName} });
        manageFeeds.find('.feedUrlInput').hostNodes().simulate('change', { target: {value: rssURL} });

        expect(manageFeeds.state().nameInput).toEqual(rssName);
        expect(manageFeeds.state().urlInput).toEqual(rssURL);

        manageFeeds.find('.addFeedButton').hostNodes().simulate('submit');
